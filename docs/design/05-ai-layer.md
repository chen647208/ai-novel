...
**内置工具首批清单**（现有 4 个 prompt 服务的"工具化"转写，不引入新功能）：
...
- **轨道二（逻辑，已落地）**：技能目录内 `handler.js`/`handler.mjs` 作为 handler 源码，经隔离沙箱（`21` 篇 S1）执行；
  能力默认拒绝——handler 只能"提议"工具调用，返回后经技能 `tools` 白名单裁决（`adjudicateHandlerResult`），
  越界调用被拒并记 capability 错误。宿主经 `core.skill.run` 工具调用逻辑轨。
...
8. 内容哈希变更触发重载、未变更不重载：已落地（`hashContent` + 装载期正文/handler 比对，未变跳过重注册）。
