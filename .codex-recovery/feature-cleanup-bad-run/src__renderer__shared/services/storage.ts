...
import { AppState, Project, StorageConfig, KnowledgeItem, EmbeddingModelConfig } from "../../types";
...
export const storage = {
  ...
  // 保存一致性检查配置
  saveConsistencyCheckConfig: async (config: import('../../types').ConsistencyCheckConfig): Promise<boolean> => {
    try {
      if (isElectron()) {
        const filePath = await getStoragePath();
        const data = await window.electronAPI!.readFile(filePath);
        const state: import('../../types').AppState = JSON.parse(data);
        state.consistencyCheckConfig = config;
        await window.electronAPI!.writeFile(filePath, JSON.stringify(state, null, 2));
      } else {
        const data = localStorage.getItem(STORAGE_FILE_NAME);
        const state: import('../../types').AppState = data ? JSON.parse(data) : {};
        state.consistencyCheckConfig = config;
        localStorage.setItem(STORAGE_FILE_NAME, JSON.stringify(state));
      }
      return true;
  ...
  // 加载一致性检查配置
  loadConsistencyCheckConfig: async (): Promise<import('../../types').ConsistencyCheckConfig | null> => {
    try {
      let state: import('../../types').AppState | null = null;
  ...
  // 保存一致性检查提示词模板
  saveConsistencyPrompts: async (prompts: import('../../types').ConsistencyCheckPromptTemplate[]): Promise<boolean> => {
    try {
      if (isElectron()) {
        const filePath = await getStoragePath();
        const data = await window.electronAPI!.readFile(filePath);
  ...
  // 加载一致性检查提示词模板
  loadConsistencyPrompts: async (): Promise<import('../../types').ConsistencyCheckPromptTemplate[] | null> => {
    try {
      let state: import('../../types').AppState | null = null;
