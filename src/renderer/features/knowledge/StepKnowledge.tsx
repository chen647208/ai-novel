    ...
    <div className="max-w-7xl mx-auto h-full p-8 flex flex-col gap-6 overflow-hidden">
      <div className="flex justify-between items-end border-b pb-6 border-gray-100">
        <div>
          ...
          <p className="text-gray-500 mt-1 italic font-medium">{t('subtitle')}</p>
        </div>
      </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2">
            {getFilteredKnowledge().length === 0 ? (
              <div className="text-center py-10 text-gray-400 text-sm italic">
                {selectedCategory === 'all' ? t('emptyAll') : t('emptyCategory', { category: getCategoryDisplayName(selectedCategory) })}
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                     ...
                     <button 
                       ...
                       title={deleteConfirmId === item.id ? t('confirmDeleteTitle') : t('deleteFileTitle')}
                     >
                       {deleteConfirmId === item.id ? (
