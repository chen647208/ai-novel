  ...
  private createTFIDFEmbedding(text: string): number[] {
    ...
    Object.entries(wordFreq).forEach(([word, freq]) => {
      ...
      if (vocabIndex !== undefined) {
        embedding[vocabIndex % this.dimensions] += freq / words.length;
      } else {
        // 否则使用哈希函数
  ...
  async clusterDocuments(
    ...
    while (centroids.length < k) {
      ...
      if (!usedIndices.has(randomIndex)) {
        const emb = embeddings[randomIndex];
        if (emb) {
          centroids.push([...emb]);
    ...
    while (changed && iterations < maxIterations) {
      ...
      for (let i = 0; i < k; i++) {
        ...
        if (cluster && cluster.length > 0 && oldCentroid) {
          const newCentroid = this.calculateCentroid(cluster.map(d => d.embedding));
          
          // 检查质心是否变化
          const oldSimilarity = this.calculateSimilarity(oldCentroid, newCentroid);
    ...
    return clusters.map((clusterDocs, index) => {
      ...
      for (let i = 0; i < clusterDocs.length; i++) {
        for (let j = i + 1; j < clusterDocs.length; j++) {
          const a = clusterDocs[i];
          const b = clusterDocs[j];
          if (a && b) {
            totalSimilarity += this.calculateSimilarity(a.embedding, b.embedding);
          }
          count++;
        }
      }
