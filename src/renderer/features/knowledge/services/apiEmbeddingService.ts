...
export class APIEmbeddingService implements EmbeddingService {
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      logger.debug('APIEmbeddingService initialize - config:', config ? {
        id: config.id,
        name: config.name,
      ...
      if (config && config.testStatus === 'success') {
        ...
        this.isReady = true;
        logger.debug(`APIEmbeddingService initialized successfully with model: ${config.modelName}, dimensions: ${config.dimensions}`);
        return true;
      }

      if (!config) {
        logger.debug('APIEmbeddingService: No active config found');
      } else if (config.testStatus !== 'success') {
        logger.debug(`APIEmbeddingService: Config test status is "${config.testStatus}", please test connection first`);
      }
  ...
  async embedText(text: string): Promise<number[]> {
    if (!this.isReady) {
      await this.initialize();
      if (!this.isReady) {
        throw new Error(i18n.t('knowledge:embedding.serviceNotReady'));
      }
    }

    if (!this.currentConfig) {
      throw new Error(i18n.t('knowledge:embedding.noActiveConfig'));
    }
  ...
  async createVectorDocuments(
    ...
    logger.debug(`成功创建 ${vectorDocuments.length} 个向量文档（来自 ${knowledgeItems.length} 个知识库条目）`);
