...
export function validateAndCompleteCardData(
  ...
  // 缺失的必填字段 = 所有"无有效值而被默认值填充"的必填字段
  // （旧实现只检查键是否存在，空字符串/空值会被漏报，导致 isValid 误判为 true）
  result.missingFields = fieldDefs
    .filter((fieldDef) => fieldDef.required && result.defaultedFields.includes(fieldDef.name))
...
export function generateFieldReport(result: FieldValidationResult): string {
  ...
  if (result.isValid && result.defaultedFields.length === 0) {
    lines.push(i18n.t('cards:report.allFilled'));
  } else {
    if (result.missingFields.length > 0) {
      lines.push(i18n.t('cards:report.missing', {
        fields: result.missingFields.join(i18n.t('cards:report.fieldJoiner')),
      }));
    }

    if (result.defaultedFields.length > 0) {
      lines.push(i18n.t('cards:report.defaulted', {
        fields: result.defaultedFields.join(i18n.t('cards:report.fieldJoiner')),
      }));
    }

    if (result.filledFields.length > 0) {
      lines.push(i18n.t('cards:report.filled', { count: result.filledFields.length }));
    }
  }
 ...
 * @returns 标准化后的值
 */
export function normalizeFieldValue(fieldName: string, value: unknown, expectedType: string): unknown {
  ...
  switch (expectedType) {
    ...
    case 'boolean':
      // Boolean('false') === true 是经典陷阱：字符串需显式解析
