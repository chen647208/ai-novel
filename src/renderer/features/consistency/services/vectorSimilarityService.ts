...
/** 相似度检测中流转的条目：metadata 在索引时携带 originalId，检索结果类型未声明该字段 */
interface SimilarityItemMetadata {
  type: string;
  name: string;
  category: string;
  originalId?: string;
...
interface SimilarityItem {
  id: string;
  content: string;
  metadata: SimilarityItemMetadata;
}
...
export async function generateWorldViewEmbeddings(
  ...
  const items: Array<{ id: string; content: string; metadata: SimilarityItemMetadata }> = [];
    ...
    try {
      await vectorIntegrationService.indexKnowledgeBase(
        ...
        [{
          ...
          addedAt: Date.now(),
          // 相似度检测复用知识库索引通道，category 取值为 character/faction/location，
          // 其中 faction/location 不在 KnowledgeCategory 联合内（历史建模遗留），此处按原行为透传
          category: item.metadata.category as KnowledgeCategory
        }]
      );
...
async function findSimilarItems(
  projectId: string,
  item: { id: string; content: string; metadata: SimilarityItemMetadata },
  threshold: number,
  maxResults: number
): Promise<Array<{ item: SimilarityItem; score: number }>> {
  try {
    // 使用向量搜索
    ...
    return results
      ...
      .map(r => ({
        item: { id: r.document.id, content: r.content, metadata: r.metadata as SimilarityItemMetadata },
        score: r.score
      }))
...
export async function performSimilarityCheck(
  ...
  // 阶段1: 生成向量
  if (onProgress) onProgress(0, 100, i18n.t('consistency:progress.generatingEmbeddings'));
  ...
  // 阶段2: 相似度检测
  if (onProgress) onProgress(30, 100, i18n.t('consistency:progress.detecting'));
    ...
    for (const char of project.characters || []) {
      ...
      for (const similar of similarItems) {
        ...
        issues.push({
          ...
          message: i18n.t('consistency:issue.charSimilarity', { a: char.name, b: otherChar.name, score: (similar.score * 100).toFixed(1) }),
          suggestion: aiAnalysis?.isDuplicate
            ? i18n.t('consistency:suggestion.mergeCharacters')
            : i18n.t('consistency:suggestion.checkCharacters'),
          commonKeywords,
          aiAnalysis
        });
      }
      ...
      processedItems++;
      if (onProgress) {
        onProgress(30 + Math.round((processedItems / totalItems) * 70), 100, i18n.t('consistency:progress.detecting'));
      }
    }
  }
    ...
    for (const faction of project.factions || []) {
      ...
      for (const similar of similarItems) {
        ...
        issues.push({
          ...
          similarityScore: similar.score,
          message: i18n.t('consistency:issue.factionSimilarity', { a: faction.name, b: otherFaction.name, score: (similar.score * 100).toFixed(1) }),
          suggestion: i18n.t('consistency:suggestion.checkFactions'),
          commonKeywords
        });
      }
    ...
    for (const location of project.locations || []) {
      ...
      for (const similar of similarItems) {
        ...
        issues.push({
          ...
          similarityScore: similar.score,
          message: i18n.t('consistency:issue.locationSimilarity', { a: location.name, b: otherLocation.name, score: (similar.score * 100).toFixed(1) }),
          suggestion: i18n.t('consistency:suggestion.checkLocations'),
          commonKeywords
        });
      }
  ...
  if (onProgress) onProgress(100, 100, i18n.t('consistency:progress.done'));
