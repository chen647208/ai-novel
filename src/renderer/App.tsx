...
import { AppState, Project, ModelConfig } from './types';
...
import { DEFAULT_PROMPTS, INITIAL_MODELS } from './constants';
...
import StepKnowledgeEnhanced from './components/StepKnowledgeEnhanced'; // 新增导入：增强版知识库组件
import StepCharacters from './components/StepCharacters';
import StepOutline from './components/StepOutline';
import StepChapterOutline from './components/StepChapterOutline';
import WritingEditor from './components/WritingEditor';
import GlobalAssistant from './components/GlobalAssistant'; // 导入全局助手
import AIHistoryViewer from './components/AIHistoryViewer'; // 新增导入：AI历史记录查看器
import VersionCheckModal from './components/VersionCheckModal'; // 新增导入：版本检查模态框
  ...
  const executeReset = async () => {
    ...
    try {
      if (type === 'factory_reset') {
        ...
        // 2. 仅清空内容
        const currentData = storage.loadState() || INITIAL_STATE;
        const cleanData = {
          ...currentData,
          projects: [],
          activeProjectId: null
                   ...
                   onClick={async () => {
                     try {
                       ...
                       const validatedState: AppState = {
                         ...
                         models: Array.isArray(newState.models) ? newState.models : INITIAL_MODELS,
                         prompts: Array.isArray(newState.prompts) ? newState.prompts : DEFAULT_PROMPTS,
