...
import { type ConsistencyCheckConfig, type ConsistencyCheckPromptTemplate } from '../../shared/types';
...
export const DEFAULT_SEMANTIC_PROMPTS: ConsistencyCheckPromptTemplate[] = [
  {
    ...
    name: '角色描述一致性检查',
    nameKey: 'consistency:defaultNames.character',
    description: '检查角色背景、性格、目标等描述是否存在矛盾',
    descriptionKey: 'consistency:defaultDescs.character',
    isDefault: true,
    applicableModes: ['ai'],
...
如果没有发现问题，返回 {"issues": []}`
  },
  {
    ...
    name: '势力行为逻辑检查',
    nameKey: 'consistency:defaultNames.faction',
    description: '检查势力设定与实际行为是否一致',
    descriptionKey: 'consistency:defaultDescs.faction',
    isDefault: true,
    applicableModes: ['ai'],
...
以JSON格式返回问题列表。如果没有问题返回 {"issues": []}`
  },
  {
    ...
    name: '地点描述一致性检查',
    nameKey: 'consistency:defaultNames.location',
    description: '检查地点描述是否存在矛盾',
    descriptionKey: 'consistency:defaultDescs.location',
    isDefault: true,
    applicableModes: ['ai'],
...
以JSON格式返回问题列表。如果没有问题返回 {"issues": []}`
  },
  {
    ...
    name: '时间线逻辑检查',
    nameKey: 'consistency:defaultNames.timeline',
    description: '检查时间线事件的逻辑顺序和因果关系',
    descriptionKey: 'consistency:defaultDescs.timeline',
    isDefault: true,
    applicableModes: ['ai'],
...
以JSON格式返回问题列表。如果没有问题返回 {"issues": []}`
  },
  {
    ...
    name: '跨引用一致性检查',
    nameKey: 'consistency:defaultNames.cross',
    description: '检查不同元素间的引用描述是否一致',
    descriptionKey: 'consistency:defaultDescs.cross',
    isDefault: true,
    applicableModes: ['ai'],
...
export const DEFAULT_SIMILARITY_PROMPTS: ConsistencyCheckPromptTemplate[] = [
  {
    ...
    name: '相似度分析判断',
    nameKey: 'consistency:defaultNames.similarity',
    description: '用于分析相似项目是否真正重复或只是相似',
    descriptionKey: 'consistency:defaultDescs.similarity',
    isDefault: true,
